...
class MITDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        return DataLoader(MITDataset(self.train_data, self.config), self.bs, shuffle=True, collate_fn=self.custom_collater, num_workers=20, drop_last=True)

    def val_dataloader(self):
        return DataLoader(MITDataset(self.val_data, self.config), self.bs, shuffle=False,collate_fn=self.custom_collater,num_workers=20, drop_last=True)
    # For now use validation until proper test split obtained
    def test_dataloader(self):
        return DataLoader(MITDataset(self.train_data, self.config), 1, shuffle=False, collate_fn=self.custom_collater, num_workers=20)
...
class MITDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x_i = x_i["img-embeddings"][0]
        x_j = x_j["img-embeddings"][0]

        experts_xi = self.load_tensor(x_i)
        experts_xj = self.load_tensor(x_j)
        ...
        return {"label":label, "path":path, "x_i_experts":experts_xi, "x_j_experts":experts_xj}
